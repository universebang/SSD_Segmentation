...
class DataGenerator:
    ...
    def parse_csv(self,
        ...
        '''
        print('type of label:', type(self.labels))
        print('length of label:', len(self.labels))
        print('self.labela[0]:', self.labels[0]) 
        print('shape of self.label[0]:', self.labels[0].shape)
        print('self.image_ids:', self.image_ids[0:10])
        print('length of self.images:', len(self.filenames))
        print('name of filename:', self.filenames[0:10])
        '''
    def parse_xml(self,
                  images_dirs,
                  image_set_filenames,
    ...
    def generate(self,
        ...
        while True:
            ...
            #########################################################################################
            def get_mask_label(dataset_dir = None):
            ...
            encoded_gtMask = get_mask_label(mask_groundth_dir)
            #print('shape of encoded_gtMask:', encoded_gtMask.shape)
            #########################################################################################
            # Compose the output.
            #########################################################################################

            print('type of batch_y_encoded:', type(batch_y_encoded))
            ret = []
            ...
            # additional option for the mask label
            if 'mask_lables' in returns: ret.append(encoded_gtMask)
    ...
    def get_class_weight(self):
        '''
        return the class weight for balancing data
        '''
        #self.cls_weight_list = np.asarray(self.cls_weight_list)
        return np.asarray(self.cls_weight_list)